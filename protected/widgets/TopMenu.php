...
class TopMenu extends CWidget
{
	...
	public function init()
	{
		...
		$route = $this->getController()->getId().'/'.$this->getController()->getAction()->getId();
		//$route=$this->getController()->getId().'/'.$this->getController()->getAction();
	...
	protected function renderMenu($items)
	{
		if(count($items))
		{
			// FP mod: disable ul-tag, it is written in the main layout
	...
	protected function renderMenuItem($item)
	{
		if(isset($item['url']))
		{
			// FP mod: added span tags to label
			$label=$this->linkLabelWrapper===null ? $item['label'] : '<'.$this->linkLabelWrapper.'>'.$item['label'].'</'.$this->linkLabelWrapper.'>';
			return CHtml::link($label,Yii::app()->params['wwwdomain'].'/'.$item['url'],isset($item['linkOptions']) ? $item['linkOptions'] : array());
		}
		else
	...
	protected function isItemActive($item,$route)
	{
		...
		if(isset($item['url']) && is_array($item['url']) && !strcasecmp(trim($item['url'][0],'/'),$route))
		{
			...
			if(isset($item['suburls']) && is_array($item['suburls'])) {
				
				foreach($item['suburls'] as $suburl) {
					if (!strcasecmp($suburl,$route)) {
						return true;
					}	
				}
