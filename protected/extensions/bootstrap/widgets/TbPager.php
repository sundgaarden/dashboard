...
class TbPager extends CLinkPager
{
	...
	public $firstPageCssClass = null;
	public $previousPageCssClass = null;
	public $internalPageCssClass = null;
	public $nextPageCssClass = null;
	public $lastPageCssClass = null;
	...
	protected function createPageButtons()
	{
		...
		if ($this->displayFirstAndLast)
			$buttons[] = $this->createPageButton($this->firstPageLabel, 0, (isset($this->firstPageCssClass) ? $this->firstPageCssClass : 'first'), $currentPage <= 0, false);

		// prev page
		if (($page = $currentPage - 1) < 0)
			$page = 0;

		$buttons[] = $this->createPageButton($this->prevPageLabel, $page, (isset($this->previousPageCssClass) ? $this->previousPageCssClass : 'previous'), $currentPage <= 0, false);

		// internal pages
		for ($i = $beginPage; $i <= $endPage; ++$i)
			$buttons[] = $this->createPageButton($i + 1, $i, (isset($this->internalPageCssClass) ? $this->internalPageCssClass : ''), false, $i == $currentPage);

		// next page
		if (($page = $currentPage+1) >= $pageCount-1)
			$page = $pageCount-1;

		$buttons[] = $this->createPageButton($this->nextPageLabel, $page, (isset($this->nextPageCssClass) ? $this->nextPageCssClass : 'next'), $currentPage >= ($pageCount - 1), false);

		// last page
		if ($this->displayFirstAndLast)
			$buttons[] = $this->createPageButton($this->lastPageLabel, $pageCount - 1, (isset($this->lastPageCssClass) ? $this->lastPageCssClass : 'last'), $currentPage >= ($pageCount - 1), false);
